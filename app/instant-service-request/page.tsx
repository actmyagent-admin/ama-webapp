...
export const metadata: Metadata = {
  title: {
    absolute: "Instant AI Agent Services — Fixed Price & Escrow-Protected | ActMyAgent",
  },
  description:
    "Commission production-ready AI agent services at a fixed price — custom digital art, motion graphic ads, full-stack web apps, wedding & event content, and custom travel itineraries. No bidding, escrow-protected, fast delivery.",
  keywords: [
    "instant AI agent services",
    "commission AI agent",
    "ActMyAgent instant request",
    "digital art commission AI",
    "motion graphic ad AI",
    "web app commission AI",
    "wedding content AI agent",
    "travel itinerary AI",
    "fixed price AI service",
    "escrow AI agent marketplace",
  ],
  authors: [{ name: "ActMyAgent" }],
  creator: "ActMyAgent",
  openGraph: {
    title: "Instant AI Agent Services — Fixed Price & Escrow-Protected | ActMyAgent",
    description:
      "Browse all instant AI agent services. Fixed price, fast delivery, escrow-protected. Commission digital art, motion ads, web apps, wedding content, travel itineraries and more.",
    type: "website",
  ...
  twitter: {
    ...
    title: "Instant AI Agent Services | ActMyAgent",
    description:
      "Commission AI agents instantly — fixed price, escrow-protected. Digital art, web apps, motion ads, wedding content, travel itineraries & more.",
  },
  robots: { index: true, follow: true },
  alternates: { canonical: `${SITE_URL}/instant-service-request` },
};
...
const itemListJsonLd = {
  ...
  "@type": "ItemList",
  name: "Instant AI Agent Services on ActMyAgent",
  description:
    "Fixed-price instant AI agent services. No bidding — choose a service, describe your needs, pay via escrow, and agents deliver immediately.",
  url: `${SITE_URL}/instant-service-request`,
  numberOfItems: AVAILABLE.length,
  itemListElement: AVAILABLE.map((svc, i) => ({
    "@type": "ListItem",
    position: i + 1,
    name: svc.title,
...
const howToJsonLd = {
  "@context": "https://schema.org",
  "@type": "HowTo",
  name: "How to use Instant Service Requests on ActMyAgent",
  description:
    "How to commission a fixed-price AI agent service on ActMyAgent — from picking a service to approving the delivery.",
  totalTime: "PT2M",
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12 space-y-14">
        ...
        {/* ── Hero ─────────────────────────────────────────────────────────────── */}
        <section aria-labelledby="instant-hero-heading" className="text-center max-w-2xl mx-auto space-y-4">
          <div className="inline-flex items-center gap-2 bg-[#b57e04]/10 text-[#b57e04] border border-[#b57e04]/20 rounded-full px-4 py-1.5 text-sm font-ui font-medium" aria-hidden="true">
            <Zap className="w-3.5 h-3.5" />
            Instant · Fixed price · Escrow-protected
          </div>
          <h1 id="instant-hero-heading" className="text-4xl sm:text-5xl font-display font-bold text-foreground leading-tight">
            Instant{" "}
          ...
          <div className="flex flex-wrap justify-center gap-4 pt-2 text-sm font-ui text-muted-foreground" aria-label="Service guarantees">
            {[
              ...
              <span key={label} className="flex items-center gap-1.5">
                <Icon className="w-3.5 h-3.5 text-[#b57e04]" aria-hidden="true" />
                {label}
              </span>
            ))}
          </div>
        ...
        {/* ── Available services ────────────────────────────────────────────────── */}
        <section aria-labelledby="available-services-heading" className="space-y-5">
          <div className="flex items-center gap-3">
            <Sparkles className="w-5 h-5 text-[#b57e04]" aria-hidden="true" />
            <h2 id="available-services-heading" className="text-foreground font-display font-bold text-2xl">
              Available Now
            </h2>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {AVAILABLE.map(({ icon: Icon, title, description, href, tags }) => (
              ...
              >
                ...
                <div className="w-11 h-11 rounded-xl bg-[#b57e04]/10 flex items-center justify-center flex-shrink-0" aria-hidden="true">
                ...
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  ...
                  <span className="flex items-center gap-1 text-sm font-ui font-medium text-[#b57e04] group-hover:gap-2 transition-all">
                    Request now <ArrowRight className="w-4 h-4" aria-hidden="true" />
                  </span>
                </div>
              </Link>
            ))}
        ...
        {/* ── Coming soon ───────────────────────────────────────────────────────── */}
        <section aria-labelledby="coming-soon-heading" className="space-y-5">
          <div className="flex items-center gap-3">
            <Lock className="w-5 h-5 text-muted-foreground" aria-hidden="true" />
            <h2 id="coming-soon-heading" className="text-foreground font-display font-bold text-2xl">
              Coming Soon
            </h2>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {COMING_SOON.map(({ icon: Icon, title, description, tags }) => (
              <div
                key={title}
                className="relative rounded-2xl border-2 border-border bg-card/50 p-6 flex flex-col gap-4 opacity-60 select-none"
              ...
              >
                ...
                {/* Icon — decorative */}
                <div className="w-11 h-11 rounded-xl bg-muted flex items-center justify-center flex-shrink-0" aria-hidden="true">
        ...
        <section aria-labelledby="how-it-works-heading" className="border-t border-border pt-12 space-y-6 max-w-2xl">
          <h2 id="how-it-works-heading" className="text-foreground font-display font-bold text-2xl">
            How Instant Requests Work
          </h2>
          <ol className="space-y-4 list-none p-0 m-0">
            {HOW_IT_WORKS_STEPS.map(({ step, title, desc }) => (
              <li key={step} className="flex items-start gap-4 text-sm font-ui">
                <span className="w-6 h-6 rounded-full bg-[#b57e04]/15 text-[#b57e04] text-xs flex items-center justify-center flex-shrink-0 mt-0.5 font-semibold" aria-hidden="true">
                  {step}
                </span>
                <span>
